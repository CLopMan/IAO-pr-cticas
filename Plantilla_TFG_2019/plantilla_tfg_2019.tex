...
\label{chap:datos}
    ...
    Los datos usados se componen de el agua total y actual en cada semana de 10
    ...
    eléctrica. A partir de estos datos también se han obtenido otros derivados:
    cambio semanal del agua actual, tasa de cambio (porcentual), y promedio móvil
    del agua actual en las últimas 4 semanas. Además, también se eliminaron los
    atributos de producción eléctrica y nombre de la cuenca, ya que son constantes
    y por lo tanto no deberían influir en el rendimiento del modelo obtenido.
    Eliminar estos atributos también permitirá obtener modelos más simples y
    rápidos de entrenar y utilizar. El atributo a predecir es el agua en la
    siguiente semana, por lo que este valor también ha añadido para el entrenamiento.
...
\label{chap:train}
    Con el fin de hacer un experimento reproducible se ha utilziado la opción de usar un único hilo con semilla local en el entrenamiento de la red neuronal. La semilla empleada ha sido 1922.
	\section{Parte 1}

        Para el entrenamiento de distintos modelos trabajaremos con variaciones sobre 3 parámetros de la red neuronal (número de capas ocultas, \textit{loss function} y \textit{learning rate}). Además, aplicaremos previamente un filtrado de datos eliminando el atributo \textit{Agua\_total}, que es constante para cada embalse. Esto resulta en 16 modelos distintos. También incluiremos un modelo extra con los valores por defecto como modelo de control.

        De esta forma, estamos empleando un \textit{grid-search} con los siguientes valores:
        \begin{itemize}
            \item Filtrado de datos: \{\textit{all}, \textit{agua total}\}.
            \item Número de capas: $\{2, 8\}$.
            \item \textit{loss function}: \{\textit{absolute}, \textit{Huber}\}.
            \item \textit{learning rate}: $\{0.05, \textit{automatic}\}$.
        \end{itemize}

    Al comparar los modelos mediante el test \textit{Anova} podemos comprobar que existen diferencias significativas entre ellos debido a que optenemos un $f-value > 1$. 
[insertar los resultados del t-test y anova-test]

    Si observamos la comparación por pares proporcionada por el \textit{t-test}, destaca el modelo J por ser el que presenta diferencias más significativas con el resto de modelos. Este modelo, además, cuenta con un RMSE de [insertar error] y una correlación de [insertar correlación]. También podemos observar que, en general, existen 2 clases de modelos. [no sé interpretar muy bien el t-test, pero creo que hay modelos similares al J y los demás con mejor error]. 

    Con toda esta información, concluimos que el mejor modelo y con el que haremos la predicción será el modelo [insertar modelo] debido a que se trata del modelo más sencillo de la segunda clase al emplear menos atributos.... [etc.].
